"""JPEG re-encoding is explicit, tiled, decoded/hashed, and metadata-clean."""
import csv
import json
...
class JpegExportTest(unittest.TestCase):
    def test_jpeg(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = anonymize_wsi(source, root / "out", compression="jpeg", include_filename=False)
            self.assertTrue(result["additional_lossy_compression"])
            self.assertFalse(result["source_compressed_payload_copied"])
            self.assertEqual(result["source_output_pixel_regions_verified"], 0)
            self.assertEqual(result["jpeg_quality"], 90)
            ...
            with tifffile.TiffFile(result["output_path"]) as tif:
                self.assertTrue(all(p.compression == 7 and p.is_tiled for p in tif.pages))
                ...
                self.assertFalse(np.array_equal(tif.pages[0].asarray(), image))
            ...
            with Path(result["csv_path"]).open(encoding="utf-8-sig", newline="") as stream:
                self.assertEqual(next(csv.DictReader(stream))["compression"], "jpeg-reencoded-q90")

            cancel = [False]
            def progress(event):
                cancel[0] = True
            with self.assertRaises(ExportCancelled):
                anonymize_wsi(source, root / "cancel", compression="jpeg", progress=progress, cancelled=lambda: cancel[0])
